...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=0.7\linewidth]{chapters/3_solvers/3_2_iterative_solvers/figures/no_preconditioner.pdf}
...
\begin{algorithm}[h]
  ...
  \KwOut{approximate solution $\iter[k]{z}$ to the system $Mz=y$\\
  \hrulealg}
  $\iter[0]{z} = 0$ \\
  \For{$i = 1$ \KwTo $k$} {
...
\subsubsection{Incomplete Factorization Preconditioners}
...
Fortunately, even if a matrix is not low-rank by itself, it might exhibit a rank structure, that can be exploited by a number of rank-structures formats such as $\mathcal{H}$-matrices (this formats will be discussed in Chapter~%TODO 
